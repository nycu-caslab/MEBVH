...
STAT_PERCENT("VQBVH/Mask false positive rate", maskFalses, maskTests);
STAT_PERCENT("VQBVH/Voxel occupancy rate", voxelSets, voxels);
...
struct alignas(16) LinearVQBVHNode {
    uint64_t mask        : 64;   
    uint8_t  seg_x0      : 4;
    uint8_t  seg_y0      : 4;
    uint8_t  seg_z0      : 4;
...
// VQBVHAggrefate Method Definitions
VQBVHAggregate::VQBVHAggregate(std::vector<Primitive> prims, int maxPrimsInNode,
                           SplitMethod splitMethod)
    : maxPrimsInNode(std::min(255, maxPrimsInNode)),
      primitives(std::move(prims)),
      splitMethod(splitMethod) {
    ...
    int offset = 0;
    bound = root->bounds;
    // bound.pMin = bound.pMin - (bound.pMax - bound.pMin) ;
    // bound.pMax = bound.pMax + (bound.pMax - bound.pMin) ;
    quantizedBounds = new Bounds3f[totalNodes];

    flattenVQBVH(root, &offset, bound);


    delete[] quantizedBounds;
    CHECK_EQ(totalNodes.load(), offset);
}

BVHBuildNode *VQBVHAggregate::buildRecursive(ThreadLocal<Allocator> &threadAllocators,
                                           pstd::span<BVHPrimitive> bvhPrimitives,
                                           std::atomic<int> *totalNodes,
                                           std::atomic<int> *orderedPrimsOffset,
                                           std::vector<Primitive> &orderedPrims) {
...
/*
BVHBuildNode *VQBVHAggregate::buildHLBVH(Allocator alloc,
                                       const std::vector<BVHPrimitive> &bvhPrimitives,
                                       std::atomic<int> *totalNodes,
                                       std::vector<Primitive> &orderedPrims) {
    // Compute bounding box of all primitive centroids
...
BVHBuildNode *VQBVHAggregate::emitLBVH(BVHBuildNode *&buildNodes,
                                     const std::vector<BVHPrimitive> &bvhPrimitives,
                                     MortonPrimitive *mortonPrims, int nPrimitives,
                                     int *totalNodes,
                                     std::vector<Primitive> &orderedPrims,
...
BVHBuildNode *VQBVHAggregate::buildUpperSAH(Allocator alloc,
                                          std::vector<BVHBuildNode *> &treeletRoots,
                                          int start, int end,
                                          std::atomic<int> *totalNodes) const {
    CHECK_LT(start, end);
...
inline Bounds3f VQBVHAggregate::computeBounds(const LinearVQBVHNode &node, const Bounds3f &parent_box) const {
    static Float lower_table[16] = {
        0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f,
        8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f
    };
    static Float upper_table[16] = {
        1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f,
        9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f
    };

    const auto &pMin = parent_box.pMin;
    const auto &pMax = parent_box.pMax;
    Point3f s, e;
    Float delta_x = (pMax.x - pMin.x) * inv_seg_interval;
    Float delta_y = (pMax.y - pMin.y) * inv_seg_interval;
    Float delta_z = (pMax.z - pMin.z) * inv_seg_interval;
    s.x = delta_x * lower_table[node.seg_x0] + pMin.x;
    s.y = delta_y * lower_table[node.seg_y0] + pMin.y;
    s.z = delta_z * lower_table[node.seg_z0] + pMin.z;
    e.x = delta_x * upper_table[node.seg_x1] + pMin.x;
    e.y = delta_y * upper_table[node.seg_y1] + pMin.y;
    e.z = delta_z * upper_table[node.seg_z1] + pMin.z;
    return Bounds3f(s, e);
}


int VQBVHAggregate::flattenVQBVH(BVHBuildNode *node, int *offset, Bounds3f parent_box) {
    LinearVQBVHNode *linearNode = &nodes[*offset];

    // compute segment bounds
    auto quantize = [](Float ds, Float de, Float p, Float q) -> std::pair<uint8_t, uint8_t> {
        Float delta = (q - p) / seg_interval;
        if(delta == 0)
            return {0, 15};
        Float temp_dsq = (ds - p) / delta;
        Float temp_deq = (de - p) / delta;
        int dsq = static_cast<int>(std::floor(temp_dsq));
        int deq = static_cast<int>(std::ceil(temp_deq));
        deq = std::max(0, deq - 1);
        dsq = std::clamp(dsq, 0, seg_interval - 1);
        deq = std::clamp(deq, dsq, seg_interval - 1);
        return {dsq, deq};
    };
    auto [seg_x0, seg_x1] = quantize(node->bounds.pMin.x, node->bounds.pMax.x, parent_box.pMin.x, parent_box.pMax.x);
    auto [seg_y0, seg_y1] = quantize(node->bounds.pMin.y, node->bounds.pMax.y, parent_box.pMin.y, parent_box.pMax.y);
    auto [seg_z0, seg_z1] = quantize(node->bounds.pMin.z, node->bounds.pMax.z, parent_box.pMin.z, parent_box.pMax.z);
    linearNode->seg_x0 = seg_x0;
    linearNode->seg_y0 = seg_y0;
    linearNode->seg_z0 = seg_z0;
    linearNode->seg_x1 = seg_x1;
    linearNode->seg_y1 = seg_y1;
    linearNode->seg_z1 = seg_z1;
    
    quantizedBounds[*offset] = computeBounds(*linearNode, parent_box);
    // CHECK(Inside(node->bounds.pMin, quantizedBounds[*offset]) == 1);
    // CHECK(Inside(node->bounds.pMax, quantizedBounds[*offset]) == 1);
    ...
    if (node->nPrimitives > 0) {
        ...
        linearNode->axis = node->splitAxis;
        linearNode->nPrimitives = 0;
                             flattenVQBVH(node->children[0], offset, quantizedBounds[nodeOffset]);
...
std::unordered_map<uint16_t, uint64_t> VQBVHAggregate::filling_lut = {
...
void VQBVHAggregate::hierarchicalMask(const Bounds3f &pBounds, LinearVQBVHNode* pNode, int cOffset, int depth){
    if(depth != 0 && nodes[cOffset].nPrimitives == 0){
        // interior node
        hierarchicalMask(pBounds, pNode, cOffset + 1, depth - 1);
        hierarchicalMask(pBounds, pNode, nodes[cOffset].offset, depth - 1);
    }
    else{
        // Aciheve limit node.
        uint64_t cmask = nodes[cOffset].mask;
        Bounds3f &cBounds = quantizedBounds[cOffset];
...
std::unordered_map<uint16_t, uint64_t> VQBVHAggregate::raymask_lut = {
...
inline bool VQBVHAggregate::maskTest(
    const Bounds3f& bounds,
    const Ray& ray,
    const LinearVQBVHNode& node,
    const Float& t0,
    const Float& t1
) const {
...
Bounds3f VQBVHAggregate::Bounds() const {
    return bound;
}

pstd::optional<ShapeIntersection> VQBVHAggregate::Intersect(const Ray &ray,
                                                          Float tMax) const {
    if (!nodes)
        return {};
    pstd::optional<ShapeIntersection> si;
    ...
    while (true) {
        ++nodesVisited;
        const LinearVQBVHNode *node = &nodes[currentNodeIndex];
        ...
        if (hit) {
            if (node->nPrimitives > 0) {
                ...
                for (int i = 0; i < node->nPrimitives; ++i) {
                    // Check for intersection with primitive in BVH node
                    pstd::optional<ShapeIntersection> primSi =
                        primitives[node->offset + i].Intersect(ray, tMax);
                    if (primSi) {
                        si = primSi;
                        tMax = si->tHit;
                    }
                ...
                --toVisitOffset;
                currentNodeIndex = nodesToVisit[toVisitOffset];
                currentBounds = nodesBounds[toVisitOffset];
            } else {
                // Put father bound on stack;
                nodesBounds[toVisitOffset] = currentBounds;
                // Put far BVH node on _nodesToVisit_ stack, advance to near node
                if (dirIsNeg[node->axis]) {
                    nodesToVisit[toVisitOffset++] = currentNodeIndex + 1;
...
bool VQBVHAggregate::IntersectP(const Ray &ray, Float tMax) const {
    if (!nodes)
        return false;
    Vector3f invDir(1.f / ray.d.x, 1.f / ray.d.y, 1.f / ray.d.z);
    int dirIsNeg[3] = {static_cast<int>(invDir.x < 0), static_cast<int>(invDir.y < 0),
    ...
    while (true) {
        ...
        if (hit) {
            ...
            if (node->nPrimitives > 0) {
                for (int i = 0; i < node->nPrimitives; ++i) {
                    if (primitives[node->offset + i].IntersectP(ray, tMax)) {
                        bvhNodesVisited += nodesVisited;
                        return true;
                    }
                }
...
VQBVHAggregate *VQBVHAggregate::Create(std::vector<Primitive> prims,
                                   const ParameterDictionary &parameters) {
    std::string splitMethodName = parameters.GetOneString("splitmethod", "sah");
    VQBVHAggregate::SplitMethod splitMethod;
    if (splitMethodName == "sah")
        splitMethod = VQBVHAggregate::SplitMethod::SAH;
    // else if (splitMethodName == "hlbvh")
    //     splitMethod = VQBVHAggregate::SplitMethod::HLBVH;
    // else if (splitMethodName == "middle")
    //     splitMethod = VQBVHAggregate::SplitMethod::Middle;
    // else if (splitMethodName == "equal")
    //     splitMethod = VQBVHAggregate::SplitMethod::EqualCounts;
    else {
        Warning(R"(BVH split method "%s" unknown.  Using "sah".)", splitMethodName);
        splitMethod = VQBVHAggregate::SplitMethod::SAH;
    }
    ...
    int maxPrimsInNode = parameters.GetOneInt("maxnodeprims", 4);
    return new VQBVHAggregate(std::move(prims), maxPrimsInNode, splitMethod);
}
...
Primitive CreateAccelerator(const std::string &name, std::vector<Primitive> prims,
    ...
    if (name == "bvh")
        ...
        accel = VQBVHAggregate::Create(std::move(prims), parameters);
    else if (name == "kdtree")
